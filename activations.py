...
def get_mels_from_audio_path(
    ...
    mels = whisper_repo.log_mel_spectrogram(audio).clone().detach().to(device) # changed from tensor(sourceTensor) to sourceTensor.clone().detach()
    return mels

class LibriSpeechDataset(torch.utils.data.Dataset):
    def __init__(self, root="speech", url="dev-clean", return_mels=False):
        super().__init__()
...
class CommonVoiceDataset(torch.utils.data.Dataset):
    def __init__(self, root="speech/cv-corpus-18.0-delta-2024-06-14", lang_code="fr", return_mels=False):
        super().__init__()
        try:
            # Load the CommonVoice dataset for the specified language
            self.dataset = torchaudio.datasets.COMMONVOICE(root=os.path.join(root, lang_code), tsv="validated.tsv")
        except RuntimeError:
    ...
    def _get_caching_hook(self, name):
        ...
        def hook(module, input, output):
            ...
            if name in self.activations:
                ...
                self.activations[f"{name}"] = cat_acts
            else:
...
def get_activations(
    ...
    if lang_code == 'fr':
        dataset = CommonVoiceDataset(root="speech/cv-corpus-18.0-delta-2024-06-14", lang_code="fr", return_mels=True)
    elif lang_code == 'en':
        dataset = CommonVoiceDataset(root="speech/cv-corpus-18.0-delta-2024-06-14", lang_code="en", return_mels=True)
    else:
        dataset = LibriSpeechDataset(return_mels=True)
    ...
    for i, (data, *labels) in enumerate(dataloader):
        ...
        for layer in activations_to_cache:
            ...
            print(f'actvs.shape: {actvs.shape}')
    ...
    for layer, actvs in all_actvs.items():
        ...
        torch.save(actvs, f"{OUT_DIR}/{model_name}_{layer}_{lang_code}")
        print(f"Saved {actvs.shape} activations for layer {layer} to disk")


# "encoder.blocks.0", "encoder.blocks.1", "encoder.blocks.2", "encoder.blocks.3",
get_activations(activations_to_cache=["decoder.blocks.0", "decoder.blocks.1", "decoder.blocks.2", "decoder.blocks.3"], model_name='tiny', lang_code='fr')
# activation_cache = WhisperActivationCache(model_name="tiny", activations_to_cache=["decoder.blocks.0", "decoder.blocks.1", "decoder.blocks.2", "decoder.blocks.3"])